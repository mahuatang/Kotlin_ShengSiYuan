 ...
 *
 * 关于数据类成员的继承要点：
 * 1.如果数据类中显示定义了equals，hashcode或是toString方法，或者是在数据类的父类中将这些方法声明为了final，
 * 那么这些方法就不会再生成，转而使用已有的；
 * 2.如果父类拥有componentN方法，并且是open的以及返回兼容的类型，那么编译器就会在数据类中生成相应的componentN方法，并且重写父类的这些方法；
 * 如果父类中的这些方法由于不兼容的签名或是被定义为final的，那么编译器就会报错。
 * 3.在数据类中显示提供componentN方法以及copy方法实现是不允许的。
 *
 * 解构声明
 *
 * 在主构造方法中有多少个参数，就会依次生成对应的component1，component2，component3...
 * 这些方法返回的是对应字段的值，componentN方法是用来实现解构声明的
 */
...
fun main(args: Array<String>) {
    ...
    person.age = 30//不要将字节码和源代码搞混，不能调用getAge方法不代表没有生成
    println(person.age)

    var person2 = person.copy(address = "hangzhou")//不加address修改的是第一个属性
    println(person2)

    var (name, age, address) = person//解构
